...
export async function generatePlayoffs(tournamentId: string) {
  ...
  await prisma.knockoutTie.createMany({ data: ties });
  return ties;
}

export async function progressKnockoutStage(tournamentId: string) {
  const allTies = await prisma.knockoutTie.findMany({
    where: { tournamentId },
    orderBy: { id: 'asc' } 
  });
  ...
  if (nextTies.length > 0) {
    await prisma.knockoutTie.createMany({ data: nextTies });
  }
