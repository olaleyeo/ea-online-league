...
exports.generateLeagueFixtures = generateLeagueFixtures;
const prisma_1 = require("../prisma");
async function generateLeagueFixtures(tournamentId) {
    const players = await prisma_1.prisma.player.findMany({ where: { tournamentId } });
    if (players.length < 8) {
    ...
    for (let i = 0; i < shuffled.length; i++) {
        ...
        for (let j = i + 1; j < shuffled.length; j++) {
            ...
            if (matchCount[p1.id] < 8 && matchCount[p2.id] < 8) {
                ...
                matchCount[p1.id]++;
                matchCount[p2.id]++;
                if (fixtures.length % (players.length / 2) === 0)
                    matchday++;
            }
        }
    }
    // Save to DB
    await prisma_1.prisma.fixture.createMany({ data: fixtures });
    // Initialize standings for all players if not exists
    for (const player of players) {
        await prisma_1.prisma.standing.upsert({
